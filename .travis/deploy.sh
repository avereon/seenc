...
chmod 600 $HOME/.ssh/*
scp -B target/product.jar travis@avereon.com:/opt/avn/store/$RELEASE/$PRODUCT/product.jar && echo "Deployed /opt/avn/store/$RELEASE/$PRODUCT/product.jar"
scp -B target/main/java/META-INF/product.card travis@avereon.com:/opt/avn/store/$RELEASE/$PRODUCT/product.card && echo "Deployed /opt/avn/store/$RELEASE/$PRODUCT/product.card"
